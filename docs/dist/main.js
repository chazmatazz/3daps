...
var animation = _dereq_('./animation.js');
...
},{"./animation.js":1,"./parser.js":3}],3:[function(_dereq_,module,exports){
function Parser() {
    ...
    function pre_process_commands(arr) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (arr[i].match(/\b(for)\b\s+\(/)) { //[A-Za-z0-9_\,\.\(\)\+\-\/\*=<>!;]+\)\s*{/)) {
                ...
                var new_commands_array = sub_array[1].split(/[;\)]/);
                arr[i] = "while (" + new_commands_array[1] + ") {";
                arr.splice(i, 0, (new_commands_array[0] + ";"));
                var end_function_index = get_index_closing_bracket(arr, i + 1);
    ...
    function go_to(func_name, args_values) {
        ...
        var func = find_function(func_name);
        if (func == null) {
    ...
    function parse_variable_values(line, values, names) {
        ...
        if ((values.length == 0) && (g_values.length == 0)) {
            return line;
        }
